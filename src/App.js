...
function App() {
  ...
  const hydrateFields = async (data, fields) => {
    ...
    const promises = fields.filter( field => (field in data && '#' in data[field]) ).map(field => {
    ...
    return Promise.all(promises).then( (fieldUpdates, index) => {
      fieldUpdates.forEach(fieldUpdate => { data[fieldUpdate.field] = fieldUpdate.data });
      return data
    })
  }
  ...
  useEffect( () => {
    ...
    //   effectCalled.current = true
      ...
      activities.map().on( (data, key) => {
        if(data !== null) { // Skip tombstoned data
          hydrateFields(data, ['last']).then( data => { updateActivityMap(key, data) })
        }
      })
    // }
            ...
            <li className="px-6 py-4" key={index}>
              <div className="flex items-center">
                ...
                <div className="grow ml-3">
                  <button className="float-right block bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onClick={() => activityDelete(key)}>Delete</button>
                  <button className="float-right block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mr-2 rounded" onClick={() => activityDone(key)}>Done</button>
                  <p className="text-sm font-medium text-gray-900">({activityDetails.cadence}) {activityName}</p>
                  <p className="text-sm text-gray-500">Last: {activityDetails.last && timeElapsed(activityDetails.last.timestamp) || "Never"}</p>
                </div>
              </div>
            </li>
          )
